...
out="$(bash "$ROOT_DIR/scripts/commands/account/list.sh" 2>&1)" || true
...
bash "$ROOT_DIR/scripts/commands/account/default.sh" | grep -q '"account_id"' || { echo "smoke_imessage: default account failed." >&2; exit 1; }

bash "$ROOT_DIR/scripts/commands/chat/list.sh" --limit=3 | grep -q '\[' || { echo "smoke_imessage: chat list failed." >&2; exit 1; }
